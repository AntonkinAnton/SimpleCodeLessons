...
namespace Polymorphism
{
    public class Program
    {
        static void Main()
        {
            var circle = new Circle(GetDouble("Введите радиус круга:"));
            ShowAreaOfShape(circle);
            Console.ReadKey(); 
            Console.Clear();

            var square = new Square(GetDouble("Введите сторону квадрата:"));
            ShowAreaOfShape(square);
            Console.ReadKey();
            Console.Clear();

            var rectangle = new Rectangle(GetDouble("Введите первую сторону квадрата:"),
